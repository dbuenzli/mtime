...
let test_available () =
  ...
  | Sys_error msg as e ->
      B0_testing.Test.log_fail "No monotonic time available: %s" msg;
      raise e

let test_pp_span () =
  B0_testing.Test.test "Mtime.pp_span" @@ fun () ->
...
let test_counters () =
  B0_testing.Test.test "counters" @@ fun () ->
  let count max =
  ...
  let do_count max =
    ...
    B0_testing.Test.log "| Count to % 8d: %10Luns %.10fs %a"
...
let test_span_constants () =
  B0_testing.Test.test "Mtime.Span.{zero,one,max_span,min_span}" @@ fun () ->
...
let test_span_arith () =
  B0_testing.Test.test "Mtime.Span.{abs_diff,add}" @@ fun () ->
...
let test_float_ns () =
  B0_testing.Test.test "Mtime.{to,of}_float_ns" @@ fun () ->
